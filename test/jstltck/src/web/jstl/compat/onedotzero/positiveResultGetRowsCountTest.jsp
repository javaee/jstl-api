...
<tck:test testName="positiveResultGetRowsCountTest">
 ...
 <sql:query var="resultSet" dataSource="${applicationScope.jstlDS}"
  ...
  <c:set var='rows' value='${resultSet.rows}' />

  <%
      SortedMap [] rows = (SortedMap[]) pageContext.getAttribute("rows");
      pageContext.setAttribute("rowsReturned", new Integer(rows.length));
  %>

  <c:choose>
      <c:when test="${rowsReturned == initParam.JSTL_TAB1_ROWS}">
      ...
      <c:otherwise>
         ...
         <c:out value='${rowsReturned}' /></strong> using the query
 ...
 <!-- RT: Validate ability to utilize Result.getRows() to access the
  ...
  <%
      rows = (SortedMap[]) pageContext.getAttribute("rows2");
