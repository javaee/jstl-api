...
/*
 * $Id: WebValidatorBase.java 56040 2008-10-15 16:16:13Z djiao $
 */
...
public abstract class WebValidatorBase implements ValidationStrategy {
    ...
    /**
     * Used to detect 4xx class HTTP errors to allow
     * fail fast situations when 4xx errors are not
     * expected.
     */
    protected static final char CLIENT_ERROR = '4';

    /**
     * Used to detect 5xx class HTTP errors to allows
     * fail fast situations when 5xx errors are not
     * expected.
     */
    ...
    /**
     * <tt>validate</tt> Will validate the response against
     * the configured TestCase.
     *
     *
     * <ul>
    ...
    public boolean validate(WebTestCase testCase) {
        ...
        // begin the check
        try {
            if (!checkStatusCode() || !checkReasonPhrase()
                || !checkExpectedHeaders() || !checkUnexpectedHeaders()
                || !checkSearchStrings() || !checkSearchStringsNoCase() || !checkUnorderedSearchStrings()
                || !checkUnexpectedSearchStrings() || !checkGoldenfile()) {
              return false;
            }
        } catch (IOException ioe) {
            TestUtil.logErr("[WebValidatorBase] Unexpected Exception: " +
                            ioe.toString());
            return false;
        }
        return true;
     ...
     *   <ul>
     *      <li>
     *         <p>If status code is -1, then return true</p>
     *      </li>
     *      <li>
     ...
     *   </ul>
     * </ul>
     * <em>NOTE:</em> If there are multiple search strings, the search
     * will be performed as such to preserve the order.  For example,
     * if the list of search strings contains two entities, the search
     * for the second entity will be started after the index if the first
     * match.
     *
     * @return boolen result of check
     * @throws IOException if an IO error occurs during validation
    ...
    protected boolean checkSearchStrings() throws IOException {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            for (int i = 0,
                 n = list.size(),
                 startIdx = 0,
                 bodyLength = responseBody.length(); i < n; i++) {
                ...
                TestUtil.logTrace("[WebValidatorBase] Scanning response for " +
                    "search string: '" + search + "' starting at index " +
                    "location: " + startIdx);
                if (searchIdx < 0) {
                    found = false;
                ...
                TestUtil.logTrace("[WebValidatorBase] Found search string: '" +
                    search + "' at index '" + searchIdx + "' in the server's " +
                    "response");
     ...
     * configured case insensitive strings that are to be expected in the response.
     *<ul>
     *  <li>
     *    Check search strings
     *  </li>
     ...
     *   <ul>
     *      <li>
     *         <p>If goldenfile is null, return true.</p>
     *      </li>
     *      <li>
     *         <p>If goldenfile is not null, compare the goldenfile
     *            with the response.  If equal, return true, otherwise
     *            display error and return false.<p>
     *      </li>
     *   </ul>
     * </ul>
    ...
    /**
     * <code>checkReasonPhrase</code> will perform comparisons between
     * the configued reason-phrase and that of the response.
     *<ul>
     *  <li>
     *    Check reason-phrase
     *  </li>
     *   <ul>
     *      <li>
     *         <p>If configured reason-phrase is null, return true.</p>
     *      </li>
     *      <li>
     *         <p>If configured reason-phrase is not null, compare the
     *            reason-phrases with the response.  If equal, return true,
     *            otherwise display error and return false.<p>
     *      </li>
     *   </ul>
     * </ul>
     *
    ...
    /**
     * <code>checkExpectedHeaders</code> will check the response for
     * the configured expected headers.
     *<ul>
     *  <li>
     ...
     *   <ul>
     *      <li>
     *         <p>If there are no expected headers, return true.</p>
     *      </li>
     *      <li>
     *         <p>If there are expected headers, scan the response for
     *            the expected headers.  If all expected headers are found,
     *            return true, otherwise display an error and return false.<p>
     *      </li>
     *   </ul>
     * </ul>
     *
    ...
    protected boolean checkExpectedHeaders() {
        ...
        if (isEmpty(expected)) {
            ...
            for (int i = 0; i < expected.length; i++) {
                ...
                Header resHeader = _res.getResponseHeader(
    ...
    /**
     * <code>checkUnexpectedHeaders</code> will check the response for
     * the configured unexpected expected headers.
     *<ul>
     *  <li>
     ...
     *   <ul>
     *      <li>
     *         <p>If there are no configured unexpected headers, return true.</p>
     *      </li>
     *      <li>
     *         <p>If there are configured unexpected headers, scan the response for
     *            the unexpected headers.  If the headers are not found,
     *            return true, otherwise display an error and return false.<p>
     *      </li>
     *   </ul>
     * </ul>
    ...
    /**
     * Utility method to determine of the expected or unexpected
     * headers are empty or not.
     */
