 ...
 */
     ...
     * @testStrategy: Validate that parseDate action can properly parse
     *                dates provided as dynamic or static String values.
     *                Pass the parsed value to formatDate to display
     ...
     *                - If type is not specified, the action properly parses
     *                  a String with the only the date component.
     *                - The value is properly parsed when explicity using
     *                  time, date, or both.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     *                 JSTL:SPEC:58.3.3; JSTL:SPEC:58.3.4; JSTL:SPEC:58.3.5;
     *                 JSTL:SPEC:58.3.6; JSTL:SPEC:58.3.7; JSTL:SPEC:58.16
     * @testStrategy: Validate the following:
     *                - Dates are parsed correctly for all values of the
     *                  dateStyle attribute.
     *                - If dateStyle is present it will only be applied
     *                  when type is not specified, or is set to date or both.
     *                - If dateStyle is not present, the action will assume the 
     *                  date to be parsed is in the default style.
     */
     ...
     *                 JSTL:SPEC:58.4.3; JSTL:SPEC:58.4.4; JSTL:SPEC:58.4.5;
     *                 JSTL:SPEC:58.4.6; JSTL:SPEC:58.17
     * @testStrategy: Validate the following:
     *                - Times are parsed correctly for all values of the
     *                  timeStyle attribute.
     *                - If timeStyle is present is will only be applied
     *                  when type is set to time or both.
     *                - If timeStyle is not present, the action will assume the 
     *                  time to be parsed is in the default style.
     */
     ...
     * @testStrategy: Validate the action will properly parse a date using
     *                the locale provided via the parseLocale attribute.
     *                This will be confirmed by setting the locale of the 
     *                page to de_DE, and pass en_US to the action itself.  
     *                No parsing exceptions should occur.
     */
     ...
     * @testStragegy: Validate that if pattern is present, it is properly
     *                applied when parsing the date value provided.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: Validate that the if var is specified and scope is not, 
     *                the parsed value is not written to the current JspWriter, 
     *                but is instead available via the PageContext and is of type
     *                java.util.Date (validation performed via custom action).
     */
     ...
     * @testStrategy: Validate that var is properly exported to the
     *                scopes as specified by the scope attribute.  Also
     *                verify that if scope is not present, that var is
     ...
     *                variable referenced by var is indeed removed.  This will
     *                be validated by setting a scoped variable before calling
     *                the action with a null or empty value, and then verifying
     *                that the var is no longer available.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: Validate that if the parseLocale attribute is
     *                null or empty, the action behaves as of the
     *                attribute was not specified.  Since setLocale
     *                was used, the parse action should use the locale
     *                from the locale configuration variable.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: Validate that the action can properly
     *                format a date based on the default I18N
     ...
     * @testStrategy: Validate that if no matching locale can be found,
     *                that the fallback locale will be used.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: Validate that if the formatting action is
     *                unable to parse the String value provided,
     *                the exception is rethrown as a JspException,
     *                with the rethrown unparsable value in the exception
     *                text and the original exception set as the root cause
     *                of the JspException.
     */
     ...
     * @testStrategy: Validate that the timeZone attribute is able to
     *                accept both String literals representing a time zone,
     *                as well as java.util.TimeZone objects.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: Validate that if timeZone is null or empty, the
     *                value will be formatted as if it was missing.
     *                In this case, the time will be formatted using
     *                the page's time zone of EST.
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: Validate that following order for determining
     *                the time zone to be used when formatting date/times.
     *                In order of precedence:
     *                  - If present, use the value of the timeZone attribute.
     *                  - If wrapped in a <fmt:setTimeZone> action, use the
     *                    timeZone of that action.
     *                  - If no timeZone attribute present, and not wrapped
     *                    by the fmt:setTimeZone action, use the value of the scoped
     *                    attribute javax.servlet.jsp.jstl.fmt.timeZone
     *                Pass the parsed value to formatDate to display
     *                (due to possible timezone difference).
     */
     ...
     * @testStrategy: validate that if var is not specified, but scope is,
