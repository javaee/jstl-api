...
/*
 * $URL: http://jse.east.sun.com:9001/re_repos/spider/branches/jstl/1.2/src/com/sun/ts/tests/jstl/spec/core/general/set/JSTLClient.java $ $LastChangedDate: 2007-02-11 23:30:10 -0500 (Sun, 11 Feb 2007) $
 */
 ...
 */
     ...
     * @testStrategy: Validate that the dynamic and static values
     *                provided to the 'value' attribute are made available
     *                to the test page by using <c:out> to print the values.
     */
      ...
      * @testStrategy: Validate the value of the exported var can
      *                be set within the body of the <c:set> action.
      *                Verify the result using <c:out>
      */
      ...
      *                     - if setting a scoped variable and value
      *                       is null, the variable referenced by var
      *                       and scope is removed.  This is validated
      *                       by setting a scoped variable, and then
      *                       calling remove using that var name.  Then
      *                       validate that the variable is indeed removed
      *                       by using an out action with that variable (which
      *                       is now null.
      *                     - If setting a property in a Map and value is null,
      *                       the key/value is removed from the map.
      *                     - If setting a property in a Bean and value is null,
      *                       will return null when calling the get method
      *                       for that propery after the set completes.
      */
      ...
      * @testStrategy: Validate that the set action can set properties
      *                of JavaBeans and set key/values in Map instances
      *                using the target and property attributes.
      */
      ...
      * @testStrategy: Validate that if the body content of the action
      *                is thrown, that it is properly propagated.
      */
      ...
      * @testStrategy: Validate that a javax.servet.jsp.JspException is
      *                thrown if the target attribute is null, or is provided
      *                an object that is no a JavaBean or an instance of
      *                java.util.Map.
      */
      ...
      * @test_Strategy: In a c:set tag, assign a deferred value to a variable.
      *                 In a second tag, set the variable to have application
      *                 scope. Verify that the value can be 
