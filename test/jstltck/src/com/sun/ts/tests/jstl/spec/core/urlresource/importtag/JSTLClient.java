...
/*
 * $URL: http://jse.east.sun.com:9001/re_repos/spider/branches/jstl/1.2/src/com/sun/ts/tests/jstl/spec/core/urlresource/importtag/JSTLClient.java $ $LastChangedDate: 2007-09-20 15:26:34 -0400 (Thu, 20 Sep 2007) $
 */
 ...
 */
     ...
     * @testStrategy: Validate that the 'url' attribute
     *                accepts both dynamic and static values.
     */
     ...
     * @testStrategy: Validate that resources identified by an
     *                absolute URL can be imported and displayed.
     *                This will test absolute URLs for HTTP and FTP
     */
     ...
     * @assertion_ids: JSTL:SPEC:23.1.1; JSTL:SPEC:23.1.1.1
     *                 JSTL:SPEC:23.20.2
     * @testStrategy: Validate that resources identified by a
     *                context-relative path can be imported and displayed.
     *                Also validate that an imported resource that sets the
     ...
     * @assertion_ids: JSTL:SPEC:23.1.1; JSTL:SPEC:23.1.1.2
     *                 JSTL:SPEC:23.20.2
     * @testStrategy: Validate that resources identified by a page-relative
     *                path can be imported and displayed.  Also validate that
     *                an imported resource that sets the status to some 2xx
     *                value doesn't cause the page to throw an Exception.
     */
     ...
     * @testStrategy: Validate that content of the imported resource
     *                is associated with the variable name defined by
     *                the 'var' attribute.
     */
     ...
     * @testStrategy: Validate that the variables defined by 'var'
     *                are exported to the appropriate scope based on
     *                the presence or absence of the 'scope' attribute.
     */
     ...
     * @testStrategy: Import the content of a JSP via a relative URL.
     *                Validate that Application, Session, Request, and
     ...
     * @testStrategy: Import the content of a JSP via an absolute URL.
     *                Validate that only the Application (Context) information
     *                is available to the target resource.
     */
     ...
     * @testStrategy: Validate that if the charEncoding attribute is set
     *                that the encoding is indeed applied to the imported
     *                resource.
     */
     ...
     * @testStrategy: Validate that if the charEncoding attribute is
     *                null or empty, the action behaves as if it was
     *                not specified.
     */
     ...
     * @testStrategy: Validate the import action can properly handle
     *                nested param subtags within the body content.
     */
     ...
     * @testStrategy: Validate the following with varReader:
     *                 - the type of the exported variable (java.io.Reader).
     *                 - the visibility of the exported variable during
     *                 - and after the import action completed (variable
     *                   is not visible after action completes).
     */
     ...
     * @testStrategy: Validate that if the imported absolute resource sends a redirect,
     *                the action properly follows the redirect and imports the target.
     */
     ...
     *                (content root without a leading slash) an
     *                Exception is thrown.
     */
     ...
     * @testStrategy: Validate that if context is specified, and the
     *                url attribute is provided an incorrect value (a path
     *                with no leading slash), an Exception
     *                is thrown.
     */
     ...
     * @assertion_ids: JSTL:SPEC:23.15
     * @testStrategy: Validate that if the value of url is empty ("")
     *                a javax.servlet.jsp.JspException is thrown.
     */
     ...
     * @assertion_ids: JSTL:SPEC:23.15
     * @testStrategy: Validate that if url is provided an invalid URL
     ...
     * @testStrategy: Validate that if an imported absolute resource
     *                has a return code other than 2xx, a JspException
     *                is thrown with the path and status code included
     *                in the Exception message.  In this case, the test
     *                will import a JSP via an absolute URL
     *                that sets the response to 410.
     */
     ...
     * @testStrategy: Validate that if an imported local resource sets
     *                a response not in the 2xx range, a JspException is
     *                thrown with the path and status code included in
     *                the Exception message.  In this case, the test
     *                will import a JSP via a relative URL that sets the
     *                response to 410.
     */
     ...
     * @testStrategy: Validate that if the included resource throws
     *                either an IOException or RuntimeException, a JspException
     *                is thrown, with the message of original exception included
     *                in the message and the original exception as the root cause.
     */
     ...
     * @testStrategy: Validate that if the included resource throws
     *                a ServletException and a root cause is present,
     *                the message of the exception text is that of the
     *                original root cause, and the root cause of the
     *                JspException is set to be the root cause of the
     *                ServletException, otherwise if no root cause is
     *                present, the message of the ServletException is
     *                used and the ServletException is the root cause
     *                of the JspException.
     */
     ...
     *                 specified by the response of the imported resource.  If
     *                 the imported resource specifies no charset, then the
     *                 default charset of ISO-8859-1 will be used.
     */
