 ...
 */
     ...
     *                localized and any messag actions outside the body
     *                are not.
     */
    ...
    public void positiveDateParamQueryTimestampTest() throws Fault {
        TEST_PROPS.setProperty(STANDARD_COMPAT, "positiveDateParamQueryTimestampTest");
        invoke();
    }
     ...
     *                attribute is present, and the formatting action is
     *                not nested in a <fmt:bundle> action, the basename attribute
     *                will take precendence over the javax.servlet.jsp.jstl.fmt.locale
     *                scoped attriubte.  This will be verfied by setting the
     *                localizationContext attribute so that it will resolve to an en_US
     *                bundle, and the set the locale attribute to de_DE.
     *                If the formatting action correctly uses the locale from
     *                the basename attribute, then no parse exception will occur.
     */
    ...
    public void positiveIfVarTest() throws Fault {
        TEST_PROPS.setProperty(STANDARD_COMPAT, "positiveIfVarTest");
        invoke();
    }
     ...
     * @assertions_ids:
     * @testStrategy: Validate that arrays of Object types
     *                can be handled by 'forEach'.
     */
     ...
     *                set of libraries is specified as a paramter to the
    ...
    public void positiveRedirectTest() throws Fault {
        ...
        TEST_PROPS.setProperty(EXPECTED_HEADERS, "Location:  http://" +
            _hostname + ":" + _port +
            "/jstl_1_0_compat_web/import.jsp");
        invoke();
        TEST_PROPS.setProperty(TEST_NAME, "positiveRedirectTest");
        TEST_PROPS.setProperty(IGNORE_BODY, "true");
     ...
     *                the configured validator, a translation error
     *                occurs if a declaration is encountered.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the behavior of the action when both
     *                scope and var are specified.  Var should
     *                be exported to the scope as specified by the scope
     *                attribute.  If scope is not present and var is,
     *                var will be exported to the page scope by default.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate sql:setDataSource exports the var attribute
     ...
     * @assertion_ids:
     * @testStrategy: Validate value can accept both
     *                String representations or locales as well
     *                as instances of java.util.Locale.
     */
     ...
     *                explicitly set scopes) by exporting different vairables
     *                to the assorted scopes and then print the values
     *                using PageContext.getAttribute(String, int).
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the sql:update action
     *                - That a SQL query can be passed as body content.
     */
