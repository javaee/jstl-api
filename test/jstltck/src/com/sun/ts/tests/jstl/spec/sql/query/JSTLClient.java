 ...
 */
     ...
     *                - That  dataSource expression can be an DataSource
     *                - Specify a simple sql query within the body of the
     *                sql:query acton and validate that you get the
     ...
     *                - That a query can be successfully executed when the
     *                dataSource attribute is passed an instance of a
     *                DataSource.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the sql:query  use of the dataSource attribute
     *                and setting it to a String representing JDBC DriverManager
     ...
     * @testStrategy: Validate the behavior of the sql:query action
     *                utilizing a sql attribute
     *                  - create a sql:query action which utilizes a sql
     *                  attribute for defining the query and validate
     *                  that you get the expected number of rows back.
     */
     ...
     * @testStrategy: Validate that the var attribute within a <sql:query>
     * action is of type javax.servlet.jsp.jstl.sql.Result.
     */
     ...
     * @testStrategy: Validate that if a sql query returns no rows,
     *                that an empty Result object is returned
     */
     ...
     * @testStrategy: Validate the  sql:query  use of the maxRows attribute.
     *                - That all rows returned if maxRows not specified.
     *                - That all rows returned if maxRows is '-1'.
     *                - That 'maxRows'rows returned if maxRows specified.
     *                - That 'maxRows' can be specified as an expression or
     *                  specified directly.
     */
     ...
     * @testStrategy: Validate the sql:query  use of the startRow attribute
     *                - That  startRow expression must be an int type
     *                - That all rows returned if startRow not specified.
     *                - That all rows returned if startRow equals 0.
     *                - That the correct starting row is returned.  This
     *                  is validated by utilizing a sorted query.
     *                - The correct number of rows is returned when startRow
     *                  is used with maxRows
     *                - That 'startRow' can be specified as an expression or
     *                  specified directly.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the  sql:query  use of the configuration
     *                parameter javax.servlet.jsp.sql.maxRows passed as a String
     *                - That the number of rows returned matches the value
     *                  specified by the config param.
     *                - That all rows are returned if the config param is '-1'.
     *                - That maxRows attribute takes precedence over the config
     *                  param.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the  sql:query  use of the configuration
     *                parameter javax.servlet.jsp.sql.maxRows passed an Integer
     *                object
     *                - That the number of rows returned matches the value
     *                  specified by the config param.
     *                - That all rows are returned if the config param is '-1'.
     ...
     *                setting it to a DataSource Object.  The query is passed
     *                as body content.
     */
     ...
     * @testStrategy: Validate the sql:query  use of the sql attribute
     *                - That a JspException is thrown when an invalid value
     ...
     * @assertion_ids: JSTL:SPEC:59.33
     * @testStrategy: Validate the sql:query  use of the maxRows attribute
     *                - That a JspException is thrown when an value < -1
     ...
     *                javax.servlet.jsp.sql.jstl.maxRows config parameter
     *                - That a JspException is thrown when an invalid value
     *                is specified.
     */
     ...
     * @testStrategy: Validate that if a sql:query  utilizes the scope
     *                attribute that is invalid, that a translation error
     *                will occur.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the sql:query action that when the var attribute
     *                is not specified, a translation error occurs.
     */
     ...
     * @assertion_ids:
     * @testStrategy: Validate the sql:query action that when the var attribute
     *                is empty, a translation error occurs.
     */
   ...
   /*
     ...
     * @assertion_ids: JSTL:SPEC:69.15
     * @testStrategy: Validate the sql:query action use of the body content
     *                - That a JspException is thrown when an invalid value
     *                is specified for sql. via body content.
     */
     ...
     * @testStrategy: Validate the sql:query  use of the dataSource attribute
     *                - That a JspException is thrown when an invalid value
     *                is specified for dataSource.
     */
    public void negativeQueryDataSourceAttributeTest() throws Fault {
        TEST_PROPS.setProperty(STANDARD,
           "negativeQueryDataSourceAttributeTest");
        invoke();
    }
   ...
   /*
     ...
     *                Object which is uninitialized for the dataSource attribute will
   ...
   /*
     ...
     * @assertion_ids:
     * @testStrategy: Validate the sql:query action which specifies null
     ...
     *   @assertion_ids: JSP:SPEC:XXX
     *   @test_Strategy: validates javax.servlet.jsp.jstl.sql.ResultSupport by using
     *   cusotm tag resultSetQuery, which invokes ResultSupport.toResult() to convert
     *   a java.sql.ResultSet to javax.servlet.jsp.jstl.sql.Result. 
     */
