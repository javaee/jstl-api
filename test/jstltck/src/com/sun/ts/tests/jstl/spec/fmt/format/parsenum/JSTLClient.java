...
/*
 * $URL: http://jse.east.sun.com:9001/re_repos/spider/branches/jstl/1.2/src/com/sun/ts/tests/jstl/spec/fmt/format/parsenum/JSTLClient.java $ $LastChangedDate: 2007-09-20 15:26:34 -0400 (Thu, 20 Sep 2007) $
 */
 ...
 */
     ...
     * @testStrategy: Validate the action can correctly parse dynamic
     *                and static values provided to the value attribute.
     */
     ...
     * @testStrategy: Validate the action can properly parse numbers
     *                provided in the three supported types (number,
     *                currency, percent).  Also verify that if type
     *                is not provided, that the value is parsed as a number.
     */
     ...
     * @testStrategy: Validate that if a pattern is specified that it will
     *                be applied to the parsed value no matter the value of type.
     */
     ...
     * @testStrategy: Validate that the action is properly able to
     *                parse the provided value based of the parseLocale attribute.
     *                Also validate that the attribute can accept either
     *                Strings or Locale objects.  Additionally validate that
     *                the presence of the parseLocale attribute will override
     ...
     * @testStrategy: Validate that if integerOnly is true, only the 
     *                integer portion of the value is returned parsed.
     *                Also validate that if integerOnly is not specified,
     *                that the entire value will be returned parsed (default
     *                value of false).
     */
     ...
     * @testStrategy: Validate that if var is specifed the parsed result
     *                is exported and is of type java.lang.Number.
     */
     ...
     * @testStrategy: Validate that var is exported to the proper scope
     *                as defined by the value of the scope attribute.  Also
     *                verify that scope is not specified, that var is exported
     *                to the page scope.
     */
     ...
     * @testStrategy: Validate that the value to be parsed can be provided
     ...
     * @testStrategy: Validate that if value is null or empty, the
     *                variable referenced by var will be removed
     *                from the scoped specified in the action.
     *                This will be verfied by setting an exported variable
     *                to the 4 given scopes, and calling the action specifying
     *                these scopes with value null or empty.  After the action
     *                completes, use the checkScope tag to validate the variable
     *                no longer exists.
     */
