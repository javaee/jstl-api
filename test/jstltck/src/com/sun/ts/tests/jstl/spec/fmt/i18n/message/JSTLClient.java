...
/*
 * $URL: http://jse.east.sun.com:9001/re_repos/spider/branches/jstl/1.2/src/com/sun/ts/tests/jstl/spec/fmt/i18n/message/JSTLClient.java $ $LastChangedDate: 2007-09-19 21:12:32 -0400 (Wed, 19 Sep 2007) $
 */
 ...
 */
     ...
     * @testStrategy: Validate that the message action is able to properly
     *                lookup and display the message associated with the provided
     *                key.  This will also establish that the key attribute
     *                can accept both static and dynamic values.
     */
     ...
     *                message from a ResourceBundle specified via the
     *                bundle attribute.
     */
     ...
     * @testStrategy: Validate that if var is present, the message is not
     *                written to the current JspWriter, but is exported
     *                to the PageContext and associated with the name
     *                specified by var.
     */
     ...
     * @testStrategy: Validate that the scope attribute affects the scope
     *                to which var is exported.  This will also validate
     *                that if var is specified, but scope is not, var is
     ...
     * @assertion_ids: JSTL:SPEC:44.4; JSTL:SPEC:44.4.1; JSTL:SPEC:44.4.2;
     * @testStrategy: Validate that if the key attribute evaluates
     *                to null, or is empty (""), that '??????' is
     ...
     * @testStrategy: Validate that if the key specified by the message
     *                action does not exist in the ResourceBundle being
     *                used, the '???<key>???' is written to the current
     *                JspWriter (<key> is the unknown key).
     */
     ...
     * @testStrategy: Validate that the message action can properly
     *                look up a localized message when the key is
     ...
     * @testStrategy: Validate that the message action can properly
     *                lookup and display compound messages with the
     *                key and <fmt:param> subtags provided as body content
     *                to the action.
     */
     ...
     * @assertion_ids: JSTL:SPEC:44.8
     * @testStrategy: Validate the the action can properly perform
     *                parametric replacement of parameters provided
     *                as body content against a compound message.
     */
     ...
     * @testStrategy: Validate that if a message is compound and no
     *                param subtags are provided, the message is returned
     *                as is.
     */
     ...
     * @testStrategy: Validate that a message action will use the prefix
     *                specified by a parent bundle action when looking up
     *                messsage keys.
     */
     ...
     *                tag and the same message action has the bundle attribute
     *                specified, that the key look up will be performed against
     *                the resource bundle specified by the bundle attribute.
     */
     ...
     * @testStrategy: Validate that if a LocalizationContext, provided
     *                to the message action has a null resource bundle, the
     ...
     *                are available, and the message action is not
     *                nested within a bundle action, the message can properly
     *                be localized.  To try to throw a wrench in things, the client will send
     ...
     * @testStrategy: Validate that if the default localization context
     *                cannot determine a locale, that the fallback locale will
     *                be used and allow the message action to properly
     *                localize a message.  To try to throw a wrench in things, the client will send
     *                a preferred locale across the wire that, if used, will
     *                not resolve to any resource bundle (no fallback defined).
     ...
     *   @test_Strategy:  validates javax.servlet.jsp.jstl.fmt.LocaleSupport
     *   for static getLocalizedMessage() methods.
     */
    ...
    public void negativeLocaleSupportTest() throws Fault {
        ...
        TEST_PROPS.setProperty(REQUEST_HEADERS, "Accept-Language: en");
        TEST_PROPS.setProperty(SEARCH_STRING, "(pageContext,key):|???nkey???|" +
