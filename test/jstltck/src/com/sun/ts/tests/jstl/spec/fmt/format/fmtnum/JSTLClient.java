...
/*
 * $URL: http://jse.east.sun.com:9001/re_repos/spider/branches/jstl/1.2/src/com/sun/ts/tests/jstl/spec/fmt/format/fmtnum/JSTLClient.java $ $LastChangedDate: 2007-09-20 15:26:34 -0400 (Thu, 20 Sep 2007) $
 */
 ...
 */
     ...
     * @testStrategy: Validate the value attribute can accept both
     *                String and Number instances as well as the
     *                acceptance of dynamic and static values.
     */
     ...
     * @testStrategy: Validate that the inclusion of the type attribute
     *                properly affects the formatting of the value.  Also
     *                validate that if type is not specified, that the value
     *                will be formatted as a number by default.
     */
     ...
     * @testStrategy: Validate that a pattern is properly applied when
     *                formatting numbers.  Also validate that if pattern
     *                is specified, that type is ignored.
     */
     ...
     *                value when type is currency.  If type is not currency,
     *                then currencySymbol should not be applied.
     */
     ...
     * @testStrategy: Validate that the currency code is properly applied
     *                to the value to be formatted when type is currency.
     *                If type is not currency, then currencyCode should
     *                not be applied.
     */
     ...
     * @testStrategy: Validate the behavior of groupingUsed.  When true,
     *                grouping separators should be applied to all format
     *                types (number, currency, percent).  If groupingUsed
     *                is not specified, groupings will be applied to the value
     *                by default.
     */
     ...
     *                different types (number, currency, percent).  The presence
     *                of the attribute should appropriately reduce the number of
     *                digits in the formatted value if the number of integer digits
     *                exceeds value of the maxIntegerDigits attribute.
     */
     ...
     *                different types (number, currency, percent).  The presence
     *                of the attribute should enforce the minimum number of digits
     *                in the final result of the format operation.
     */
     ...
     *                fractional digits exceeds value of the maxIntegerDigits
     *                attribute.
     */
     ...
     *                different types (number, currency, percent).  The presence
     *                of the attribute should enforce the minimum number of
     *                fractional digits in the final result of the format
     *                operation.
     */
     ...
     * @assertion_ids: JSTL:SPEC:55.10
     * @testStrategy: Validate that the formatNumber action exports the
     *                formated result and can be referenced by the variable
     *                name specified by var.  Also validate that the type
     *                of the scoped variable is java.lang.String.
     */
     ...
     * @testStrategy: Validate that the action exports var to the
     *                specified scope as well as validating that if scope
     ...
     * @testStrategy: Validate that if the value attribute is provided
     *                with a null or empty value, that no action is performed.
     */
     ...
     * @testStrategy: Validate that the value to be formatted can be provided
     ...
     * @testStrategy: Validate the following with both currencyCode
     *                and currencySymbol defined:
     *                 - Using en_US as the Locale.
     *                 - If runtime is 1.4 or greater currencyCode will
     *                   take precedence over currencySymbol.  In this
     *                   case a '$' will prefix the formatted value.
     *                 - If runtime earlier than 1.4, currencySymbol will
     *                   take precedence over currencyCode.  In this case
     *                   USD should prefix the formatted value.
     */
