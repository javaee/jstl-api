 ...
 */
/*
 * $Id: TSRuntimeConfiguration.java 57853 2009-05-06 15:03:59Z kgrucci $
 */
 ...
 * substituted values) in the harness.temp.directory/tmp directory.  Each
 * time a given test is run, this copy is only created iff the jte file or
 * the original runtime have been changed since the last writing of the copy.
 *
 * @author	Kyle Grucci
 */
public class TSRuntimeConfiguration {
    ...
    public TSRuntimeConfiguration(PrintWriter out) throws PropertyNotSetException {
        ...
        securedWebPort = propMgr.getProperty("securedWebServicePort", securedWebPort);
        securedWebPort2 = propMgr.getProperty("securedWebServicePort.2", securedWebPort2);
        ...
        namingServiceHost1 = propMgr.getProperty("namingServiceHost1", namingServiceHost1);
        namingServiceHost2 = propMgr.getProperty("namingServiceHost2", namingServiceHost2);
        namingServicePort1 = propMgr.getProperty("namingServicePort1", namingServicePort1);
        namingServicePort2 = propMgr.getProperty("namingServicePort2", namingServicePort2);
    ...
    public String sweepRuntimeFile(File file) throws FileNotFoundException, IOException {
        ...
        return replacer.replace(file, htReplacementProps, htReplacerTable, sTempDir);
    }
    ...
    final class StringReplacer {
        ...
        public String replace(File file, Hashtable htStrings, Hashtable htReplacerTable, String sTempDir) {
            ...
            try {
                ...
                /*  Always copy the file to tmp even if we don't replace
                 *  anything as it makes the location consistent for xml
                 *  deployment handlers.
                 */
        ...
        class ReplacementInfo {
            ...
            public void foundOccurance() {
                iFoundOccurances++;
                if (TestUtil.harnessDebug)
                    TestUtil.logHarnessDebug("we found an occurance #" + iFoundOccurances + " of '" + sFind + "'");
            }
        }
    }

    /**
     * Parse the HashTable which contains strings retrieved via calls into
     * the porting classes for deployment.  We must substitute on these strings
     * prior to substituting them into the runtime.xml. 
     */
        ...
        for (Enumeration e = extras.keys(); e.hasMoreElements();) {
            ...
            for (int i = 0; i < searchFor.length; i++) {
                ...
                if ((startPos = val.lastIndexOf(searchFor[i])) != -1) {
                    ...
                    val = startBuff + buff + val.substring(startPos + searchFor[i].length());
                }
            }
            if (changeIt) {
