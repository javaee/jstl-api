...
/*
 * $Id: TSTestFinder.java 56505 2009-01-20 16:31:57Z phendley $
 */
 ...
 * for J2EE-TS testing.
 *
 * A test description consists of a single block comment in Java files.
 * A file may contain multiple test descriptions.
 *
 * @author Vella Raman
...
// ------------------------------------------------------------- Instance fields
    ...
    // COMMENTED OUT AS THEY ARE NEVER USED
    // private static final String LINESEP = System.getProperty("line.separator");
    // private static boolean cachedKeyList;
    // private File currFile;
    // private Hashtable excludeTable = new Hashtable();
    // private Hashtable keyList = new Hashtable(3);
    // private boolean testprop;
    // private Hashtable propTags;
...
// ----------------------------------------------------------- Protected Methods
    ...
    /**
     *  Override the scan method in TagTestFinder since we have to
     * deal with CommentStreams that are valid test tags ( but not
     * a test description object of themselves -- the class.setup_props
     * tag ).
     * @param file The file to scan
     */
    protected void scanFile(File file) {
        ...
        if (!name.endsWith(".java") ||
             name.equals("BaseUIComponentClient.java") ||
        ...
        if (csc == null) {
            String[] msgs = {
                "no parser found for specified file", "file: " + file, "extension: "
                + extn
            };
        ...
        try {
            ...
            String[] msgs = {
                "problem instantiating class for extension", "extension: " + extn,
                "class: " + csc.getName()
            };
            TestUtil.logErr(msgs[0] + msgs[1] + msgs[2]);
            return;
            ...
            String[] msgs = {
                "illegal access while instantiating class for extension", "extension: "
                + extn, "class: " + csc.getName()
            };
            TestUtil.logErr(msgs[0] + msgs[1] + msgs[2]);
            return;
        }
        try {
            ...
            sInteropDirections = InteropTestManager.getInteropDirections(file.getPath());
        ...
        if (!entries.isEmpty()) {
            ...
            if (sVal.length > 0) {
                for (int ii = 0; ii < sVal.length; ii++) {
                    if (sInteropDirections.equals(INTEROP_FORWARD) || sInteropDirections.equals(INTEROP_BOTH)) {
                        //create 1 desc for each vehicle to be tested
                        td = createTestDescription(entries, file, sVal[ii].trim(),
                            false);
                        foundTestDescription(td);
                    }
                    if (sInteropDirections.equals(INTEROP_REVERSE) || sInteropDirections.equals(INTEROP_BOTH)) {
                        //create 1 desc for each vehicle to be tested
                        td = createTestDescription(entries, file, sVal[ii].trim(),
                            true);
                        foundTestDescription(td);
...
// ------------------------------------------------------------- Private Methods
    ...
    private TestDescription createTestDescription(Map tags, File file, String sVehicle,
                                                  boolean bReverse) {
        ...
        for (Iterator e = tags.keySet().iterator(); e.hasNext();) {
            ...
            if ((key.equals(KEY_TEST_NAME)) || (key.equals(KEY_CUSTOM))) {
                ...
                if (ASSERTION_COVERAGE) {
                    //use the original value as specified in source code for coverage tool
                    desc.put(key, sval);
                } else {
                    desc.put(key, AssertionMapper.getAssertionDescriptions(sval,
                        file));
                }
            ...
            if ((key.equals(KEY_CLASS_SETUP_PROPS)) || (key.equals(KEY_TEST_CASE_SETUP_PROPS))) {
                sval = (String) tags.get(key);
                TestUtil.logHarnessDebug("KEY_TEST_CASE_SETUP_PROPS sval \""
					 + sval + "\"");
                if (sval == null)
                    continue;
                sval = sval.trim();
		...
		if (!sval.endsWith(";")) {
 		    sval = sval + ";";
                ...
                TestUtil.logHarnessDebug("KEY_TEST_CASE_SETUP_PROPS normalized sval \""
					 + sval + "\"");
        ...
        //add keywrods for forward and reverse tests
        if (bReverse) {
            sKeywords += "reverse";
        } else {
            sKeywords += "forward";
    ...
    private String getPkgName2(File tfile) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                line = line.trim();
                if (line.length() > 0 && line.startsWith("package") && line.endsWith(";")) {
