...
/*
 * $Id: InteropTestManager.java 52502 2007-01-24 02:50:22Z lschwenk $
 */
...
public class InteropTestManager {
    ...
    /**
     * This method returns the directions that a given interop test is to be run
     * in based on the javatest keywords.  Possible values are forward, reverse, or both.
     *
     * @return			the direction(s) to run - forward, reverse, or both.
     */
    public static String getInteropDirectionsFromKeywords (String sPath) {
        ...
        //if kewords contains all, then return default interopDirections for the directory
        //if keywords contains forward but not reverse or all, and the default == forward or both, then return forward
        //if keywords contains reverse and not forward or all, and the def == reverse or both, then return reverse
        ...
        if (jteMgr != null)
        {
            ...
            if(keywords.contains("forward") && 
              !keywords.contains("reverse") &&
              !keywords.contains("all") &&      
              (defaultDirections.equals("forward") || defaultDirections.equals("both")))
            {
                sDirections = "forward";
            }
            else if(keywords.contains("reverse") &&
                   !keywords.contains("forward") &&
                   !keywords.contains("all") &&
                   (defaultDirections.equals("reverse") || defaultDirections.equals("both")))
            {
                sDirections = "reverse";
            }  
        }
    ...
    private static String convertSlashesToUnderscores (String sTestDir) {
        String sRelativeTestPath = (sTestDir.substring(sTestDir.indexOf(File.separator + "ts" + File.separator +
                "tests" + File.separator) + 4)).replace(File.separatorChar, '_');
