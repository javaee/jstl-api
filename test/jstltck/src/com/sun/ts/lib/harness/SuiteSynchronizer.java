 ...
 */
/*
 * $Id: SuiteSynchronizer.java 57316 2009-03-24 16:49:00Z kgrucci $
 */
...
public class SuiteSynchronizer {
    ...
    private void init() {
		...
		if ((htTSDeployers.get("cts1") == null)) {
		    htTSDeployers.put("cts1", TSDeployment.getDeploymentInstance(logOut, "porting.ts.deploy.class.1"));
		}
		if (bSupportsAutoJMS && (htTSJMSAdmins.get("cts1") == null)) {
		    htTSJMSAdmins.put("cts1", TSJMSAdmin.getTSJMSAdminInstance(logOut, "porting.ts.jms.class.1"));
		}
		if (bSupportsInterop) {
		    //get porting class 2 impls for deployment and JMSAdmin
		    if (htTSDeployers.get("cts2") == null) {
			htTSDeployers.put("cts2", TSDeployment.getDeploymentInstance(logOut, "porting.ts.deploy.class.2"));
		    }
		    if (bSupportsAutoJMS && (htTSJMSAdmins.get("cts2") == null)) {
			htTSJMSAdmins.put("cts2", TSJMSAdmin.getTSJMSAdminInstance(logOut, "porting.ts.jms.class.2"));
		    }
		}
            } catch (Exception e) {
                e.printStackTrace();
        ...
        if (!vCommonAppsDeployedThisJVM.isEmpty()) {
            for (int ii = 0; ii < vCommonAppsDeployedThisJVM.size(); ii++) {
                ...
                try {
                    ...
                    TestUtil.logHarness("Failed to complete TS common application deployment.");
                    TestUtil.logHarness("*******************************");
                    throw new TSDeploymentException("Exception:  Failed to Undeploy common apps in " + testDir, e);
                }
            }
    ...
    public void doDeployment(String testDirectory, Properties props)
        ...
        //        testDir = TestUtil.srcToDist(TSTestFinder.getAbsolutePath(td.getParameter("test_directory")));
            ...
            for (int ii = 0; sCommonApps != null && ii < sCommonApps.length; ii++) {
                ...
                if (!vCommonAppsDeployedThisJVM.contains(sCommonTestDir)) {
                    try {
                        ...
                        logOut.println("Failed to undeploy common apps in " + sCommonTestDir);
                        throw de;
                    } catch (Exception e) {
                        sDeployStatus = "failed";
                        logOut.println("Unexpected Exception:  Failed to undeploy common apps in " + sCommonTestDir);
                        throw new TSDeploymentException(
                            "Exception:  Failed to undeploy common apps in " + sCommonTestDir,
                            e);
                    }
                    ...
                    if (sCommonTestDir.indexOf("txECMPbean") != -1
                        || sCommonTestDir.indexOf("txEPMbean") != -1
                        || sCommonTestDir.indexOf("ejbql" + File.separator + "schema") != -1) {
                        if (TestUtil.harnessDebug)
                    ...
                    try {
                        sCommonClientClassesPath += deployApps(sCommonTestDir) + File.pathSeparator;
                        if (TestUtil.harnessDebug)
                    ...
                    if (TestUtil.harnessDebug)
                        TestUtil.logHarnessDebug(
                            "Common apps from this dir are already deployed during this JVM- " + sCommonTestDir);
                }
            }
        } else {
            if (TestUtil.harnessDebug)
        ...
        if (!Boolean.getBoolean("common.apps.only") && !sLastTestDirectory.equals(testDir)) {
            ...
            try {
                ...
                sDeployStatus = "failed";
                logOut.println("Exception:  Undeployment of current apps in " + testDir + "failed");
                sDeployStackTrace = TestUtil.printStackTraceToString(e);
                throw new TSDeploymentException("Exception:  Undeployment of current apps in " + testDir + "failed", e);
            }
            try {
                ...
                sDeployStatus = "failed";
                TestUtil.logHarness("Deployment of app(s) from:  " + testDir + " failed!");
                sDeployStackTrace = TestUtil.printStackTraceToString(de);
                throw de;
            } catch (Throwable e) {
                sDeployStatus = "failed";
                TestUtil.logHarness("Exception:  Deployment of app(s) from:  " + testDir + " failed!");
                e.printStackTrace(logOut);
                sDeployStackTrace = TestUtil.printStackTraceToString(e);
                throw new TSDeploymentException("Deployment of app(s) from:  " + testDir + " failed!", e);
            }
        }
        sDeployStatus = "passed";
    }
    ...
    public void removeJmsConnectionFactories() throws TSJMSAdminException {
        ...
        try {
            if (bSupportsAutoJMS) {
                ...
                logOut.println("Successfully removed JMS connection factories from server 1.");
                ...
                logOut.println("Successfully removed JMS connection factories from server 2.");
            }
        } catch (Exception e) {
    ...
    public void undeployLastApp() throws TSDeploymentException, TSJMSAdminException {
        try {
            if (!sLastTestDirectory.equals("none run yet")) {
                ...
                undeployApps(sLastTestDirectory);
                logOut.println("Undeployed apps from - " + sLastTestDirectory);
                //reset this string in case we're running the GUI and we rerun the same tests
    ...
    /**
     * Uses different approaches to search for runtime files for j2sdkee RI and for s1as.
     * For j2sdkee RI, iterates through *.runtime.xml, and for each runtime.xml file, checks
     * if it matches a jar entry.
     * For s1as, search for a list of required runtime files for the current deployable component,
     * and tries to find it in the collection of all runtime files in the current dist dir.  If not
     * found, tries to get the generic forms from $TS_HOME/src/com/sun/ts/tests/common/sunxml.
     * Available generic forms of s1as runtime files:
     * sun-application.xml
     * sun-web.xml
     ...
     * TODO: use File/File[] to represent a file or files, instead of strings.  With strings
     * we don't know if they are file name or full path.
     * TODO: use collection instead of arrays.
     * TODO: remove sRuntimeInfoArray from param list.  Since we already have sDir (dist dir)
     * in param list, we can include runtime files as needed, instead of
     * calling File.list() before every invocation of getValidRuntimeInfoFiles.
     */
    private String[] getValidRuntimeInfoFiles(
        String sAppEar,
        String[] sRuntimeInfoArray,
        String sDir,
        Hashtable oppositeServerJNDITable,
        boolean bSweep)
        throws TSDeploymentException {

        String[] sValidRuntimeInfo = null;
        sValidRuntimeInfo = getS1ASRuntimeInfoFiles(sAppEar, sRuntimeInfoArray, sDir);
        
        //sweep runtime info for both cases: using merged one single runtime, or multiple
    ...
    /**
     * Gets the basename from a path.  For example, c:\foo\foo.ear would return foo.ear,
     * c:\foo\foo.bar.ear would return foo.ear, foo.ear returns foo.ear.  We could have used File.getName if we have a file
    ...
    /**
     * gets the valid s1as runtime files from the list of all available runtime files in the current test
     * dir.  If it uses generic forms of runtime files, like sun-application.xml, sun-web.xml, they will 
    ...
    private String[] getS1ASRuntimeInfoFiles(String sAppEar, String[] sRuntimeInfoArray, String sDir) {
        ...
        try {
            if (sAppEar.endsWith(".ear")) { //need sun-application.xml 
                addIfPresent(baseName, al, sRuntimeInfoArray);
                JarFile earFile = new JarFile(sAppEar);
                for (Enumeration enum1 = earFile.entries(); enum1.hasMoreElements();) {
                    ...
                    if (!infile.isDirectory()) {
                        ...
                        if (name.endsWith(".war") || name.endsWith("_ejb.jar") || name.endsWith("_client.jar")) {
                            addIfPresent(name, al, sRuntimeInfoArray);
                        }
                    }
                } //for
     ...
     */
    private void addIfPresent(String moduleName, List al, String[] runtimeFileNames) {
        for (int i = 0; i < runtimeFileNames.length; i++) {
            if (runtimeFileNames[i].startsWith(moduleName)) {
                al.add(runtimeFileNames[i]);
                if(TestUtil.harnessDebug) {
                    TestUtil.logHarnessDebug("valid s1as runtime file for:" + moduleName 
                    + ": " + runtimeFileNames[i]);
                }
            }
        }
           ...
           if (executionMode == ExecutionMode.DEPLOY_RUN_UNDEPLOY
            ...
            if (sInteropDirections.equals("forward") || sInteropDirections.equals("both")) {
                ...
                {	    
                    ...
                    //This is only for the case that we are a rebuildable test and the direction
                    //is forward or both.  In this case, we must swap settings prior to
                    //runtime file substitution which happens in populatePortingReplacementTables
            ...
            if (sInteropDirections.equals("reverse") || sInteropDirections.equals("both")) {
                ...
                {	    
                    ...
                    //This is only for the case that we are a rebuildable test and the direction
                    //has been overridden to reverse.  We don't swap settings and deploy in the
            ...
            if (TestUtil.harnessDebug)
                TestUtil.logHarnessDebug(
                    "Deployment of apps disabled due to execute mode being set to:" + String.valueOf(executionMode));
        }
    ...
    private DeploymentInfo getDeploymentInfo(
        ...
        try {
            ...
            e.printStackTrace();
            throw new TSDeploymentException(
                "Exception reading runtime information from Deliverable instance: " + e.getMessage());
        }
    }
    ...
    private void populatePortingReplacementTables(String sDir, String[] sAppJarsArray) throws TSDeploymentException {
        ...
        String[] sRuntimeInfoFilesArray =
            fTestDir.list(RuntimeInfoFilter.getInstance());
        ...
        for (int ii = 0; ii < sAppJarsArray.length; ii++) {
            ...
            sValidRuntimeInfoFilesArray =
                getValidRuntimeInfoFiles(
                    sDir + File.separator + sAppJarsArray[ii],
                    sRuntimeInfoFilesArray,
                    sDir,
                    null,
                    false);
            infoArray[ii] =
        ...
        //a different dir?
        //Make sure that htJNDIMapsFromServer1 always matches the porting class.1 and the same for 2
        if (bReversed) {
    ...
    private String continueToDeployApps(String sDir, String[] sAppJarsArray) throws TSDeploymentException, TSJMSAdminException {
        ...
        for (int ii = 0; ii < sAppJarsArray.length; ii++) {
            ...
            /*}
                ...
                // get only those runtime files which go with this
                // .ear file
                sValidRuntimeInfoFilesArray = getValidRuntimeInfoFiles(sDir + File.separator
                        + sAppJarsArray[ii], sRuntimeInfoFilesArray, sDir, null,
                        true);
            }*/
            deploymentInfo =
                getDeploymentInfo(earFile, sAppJarsArray[ii], sFileNameMinusExtension, sValidRuntimeInfoFilesArray);
            //save off all common app infos so that they can be passed to new test directory deployments
            ...
            //give the current ear's deployment info access to previosly deployed info objects
            deploymentInfo.setPreviousInfos(
                (DeploymentInfo[]) vCurrentDeploymentInfos.toArray(new DeploymentInfo[vCurrentDeploymentInfos.size()]));
            //deploy
            ...
            try {
                if (sFileNameMinusExtension.indexOf("_j2ee2") != -1 ||
                    sFileNameMinusExtension.startsWith("vi_built_")) {
                    deploymentInfo.setProperty("deployment_host", jteMgr.getProperty("deployment_host.2"));
                    ...
                    if (bSupportsAutoJMS) {
                        ...
                        if (bCreateConnectionFactories2 && TSJMSAdmin.requiresJmsFactories(sDir)) {
                            Hashtable htTopicFactories = TSJMSAdmin.getTopicConnectionFactories(2);
                            ...
                            Hashtable htQueueFactories = TSJMSAdmin.getQueueConnectionFactories(2);
                    ...
                    deploymentInfo.setProperty("deployment_host", jteMgr.getProperty("deployment_host.1"));
                    ...
                    if (bSupportsAutoJMS) {
                        ...
                        if (bCreateConnectionFactories1 && TSJMSAdmin.requiresJmsFactories(sDir)) {
                            Hashtable htTopicFactories = TSJMSAdmin.getTopicConnectionFactories(1);
                            int iSize = htTopicFactories.size();
                            ...
                            Hashtable htQueueFactories = TSJMSAdmin.getQueueConnectionFactories(1);
                            iSize = htQueueFactories.size();
    ...
    private void undeployApps(String sDir) throws TSDeploymentException, TSJMSAdminException {
        if (executionMode == ExecutionMode.DEPLOY_RUN_UNDEPLOY || executionMode == ExecutionMode.UNDEPLOY) {
            String sInteropDirections = InteropTestManager.getInteropDirectionsFromKeywords(sDir);
            ...
            if (TestUtil.harnessDebug)
                TestUtil.logHarnessDebug(
                    "Undeployment of apps disabled due to execute mode being set to:" + String.valueOf(executionMode));
        }
    }
    ...
    private void continueToUndeployApps(String sDir, String[] sAppJarsArray) throws TSDeploymentException, TSJMSAdminException {
        ...
        for (int ii = 0; ii < sAppJarsArray.length; ii++) {
            ...
            if (TestUtil.harnessDebug)
                TestUtil.logHarnessDebug(
                    sFileNameMinusExtension + " ear_file = " + sDir + File.separator + sAppJarsArray[ii]);
            try {
                if (sFileNameMinusExtension.indexOf("_j2ee2") != -1 ||
                    sFileNameMinusExtension.startsWith("vi_built_")) {
                    pDeployProps.setProperty("deployment_host", jteMgr.getProperty("deployment_host.2"));
                    ...
                    pDeployProps.setProperty("deployment_host", jteMgr.getProperty("deployment_host.1"));
            ...
            if (earFile.endsWith(".rar")) {
                ...
                try {
                    Properties connectorProps = new Properties();
                    connectorProps.setProperty("rar_file", earFile);
                    connectorProps.setProperty("deployment_host", pDeployProps.getProperty("deployment_host"));
                    connectorProps.setProperty(
                        "deployment.props.number",
                ...
                if (ctsDeploy.isDeployed(pDeployProps)) {
                    ...
                    logOut.println("Successfully undeployed app:  " + sDir + File.separator + sAppJarsArray[ii]);
                    if (bSupportsAutoJMS) {
    ...
    public static class RuntimeInfoFilter implements FilenameFilter {
        ...
        public boolean accept(File dir, String name) {          
            ...
            if ((name.endsWith(".xml") && (name.indexOf(".sun-") != -1 || name.startsWith("sun-"))) ||
