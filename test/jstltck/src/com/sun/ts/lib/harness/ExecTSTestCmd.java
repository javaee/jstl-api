...
/*
 * $Id: ExecTSTestCmd.java 52502 2007-01-24 02:50:22Z lschwenk $
 */
...
public class ExecTSTestCmd extends ProcessCommand
{
     ...
     *				By convention, the expected values are:
     *				<table>
     *				<tr><td>95    <td>passed
     ...
     *				stderr before exiting the process.   If it is not
     *				received as the last line, the value will be lost.
     *  @return			If logStatus is null, the result is determined from
     *				just the exitCode; if the logStatus is not null,
     *				the type of the status must correspond to the 
     *				standard exit code for that type, in which case the
     *				result will be logStatus; otherwise an object
     *				will be returned indicating an error.
     *			
     **/
    protected Status getStatus(int exitCode, Status logStatus) {
	// first, convert the exit code back into a Status type code
	int typeFromExitCode = 
	    (exitCode == Status.exitCodes[Status.PASSED] ? Status.PASSED :
	     exitCode == Status.exitCodes[Status.FAILED] ? Status.FAILED :
	     -1 );
	...
	if (logStatus == null) {
		...
		return Status.failed("unexpected exit code (" + 
				     exitCode + 
				     ") for reported status (" +
				     logStatus.toString() + 
				     ")");
    ...
    private static final Status defaultStatus[] = {
	Status.passed("OK"),
