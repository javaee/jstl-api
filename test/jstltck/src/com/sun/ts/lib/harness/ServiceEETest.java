...
/*
 * $Id: ServiceEETest.java 52502 2007-01-24 02:50:22Z lschwenk $
 */
 ...
 * etc.  When a service test is encountered by the JavaTest Client,
 * the instance is passed to a J2EE server component and run from
 * that remote location.  Using this model to develop tests allows
 * the same test to be run from different locations within the scope
...
public abstract class ServiceEETest extends EETest
{
    ...
    /**
     * Returns any additional properties that may need to be set 
     * by a subclass of ServiceEETest for use by a specific vehicle.
     * This method was created due to a need for clients of the
     * JBIVehicle to set the name of the object to lookup in the
     * rmiregistry.  By rule, this value should match the id name
     * for the component specified in the JBI installation
     * descriptor.  This impl returns an empty properties object
     * by default.
     * 
     * @param  p	   user configured properties used by the test
     * @return Properties  Additional properties that may need to 
     *                     be set by a subclass of ServiceEETest
    ...
    /**
     * When called within the harness VM, this method passes an instance
     * of itself to the appropriate J2EE server component.  When
     * called from within that server component, EETest's run method
     * is called and the test is run.
     *
    ...
    public Status run (String[] argv, Properties p) {
        ...
        if (TestUtil.iWhereAreWe == TestUtil.VM_HARNESS  &&
            ...
            //we're on the server in a generic vehicle, so just call on EETest
    ...
    public Status run (String[] argv, PrintWriter log, PrintWriter err) {
        ...
        try {
            iLogDelaySeconds = Integer.parseInt(props.getProperty("harness.log.delayseconds",
                    "1"))*1000;
        ...
        if (sTestCase == null)
            ...
            for (int ii = 0; ii < sVehicles.length; ii++) {
                ...
                TestUtil.setCurrentTest(sTestCase, new PrintWriter(log, true), new PrintWriter(err,
                        true));
