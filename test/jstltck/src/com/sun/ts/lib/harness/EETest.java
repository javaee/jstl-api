...
/*
 * $Id: EETest.java 52502 2007-01-24 02:50:22Z lschwenk $
 */
...
/**
 * This abstract class must be extended by all clients of all J2EE-TS
 * tests.  All implementations of this class must define a setup, cleanup,
 * and runtest(method names of runtest methods must match the 'testname'
 * tag.  EETest uses reflection to invoke these methods which in turn,
 * run the test(s) to completion.  Tests are assumed to pass, unless
 * a Fault is thrown.
 *
 * @author	Kyle Grucci
 */
public abstract class EETest
    ...
    /*
     * Please do NOT change this class in an incompatible manner with respect to serialization.
     * Please see the serialization specification to determine what is a compatible change
     * versus incompatible.  If you do need to change this class in an incompatible manner you will
     * need to rebuild the compat tests.  You should also increment the serialVersionID field to
     * denote that this class is incompatible with older versions.
     */
    static final long serialVersionID = -4235235600918875382L;
    ...
    private Method getSetupMethod (Class testClass) {
        String[] s =  {};
        Class[] setupParameterTypes =  {
            s.getClass(), (new Properties()).getClass()
        };
        ...
        try {
            ...
            try {
                ...
                setTestStatus(Status.failed("Could not find setup method for" +
                        "testcase: " + sTestCase), e2);
            }
        } catch (RuntimeException e) {
            setTestStatus(Status.failed("Could not access the test case: " + sTestCase),
                    e);
        } catch (ThreadDeath t) {
            throw  t;
        } catch (Throwable t) {
    ...
    private Method getRunMethod (Class testClass) {
        ...
        try {
            ...
            setTestStatus(Status.failed("Could not find the run method" + "for test case: "
                    + sTestCase), e);
        } catch (RuntimeException e) {
            setTestStatus(Status.failed("Could not access the test case: " + sTestCase),
                    e);
    ...
    private Method getCleanupMethod (Class testClass) {
        ...
        try {
            ...
            try {
                ...
                setTestStatus(Status.failed("Could not find cleanup method for" +
                        "testcase: " + sTestCase), e2);
            }
        } catch (RuntimeException e) {
            setTestStatus(Status.failed("Could not access the test case: " + sTestCase),
    ...
    /**
     * <p>This method is only called when test are run outside of JavaTest.
     * If a testcase name is passed within argv, then that testcase is run.
     * Otherwise, all testcases within this implementation of EETest are run.</p>
    ...
    /**
     * This method is only called when tests are run outside of JavaTest or
     * if the test is being run in the same VM as the harness.
     * If a testcase name is passed within argv, then that testcase is run.
     * Otherwise, all testcases within this implementation of EETest are run.
     *
     * @param	argv	an array of arguments that a test may use
    ...
    /**
     * This method is only called from JavaTest to run a single testcase.
     * All properties are determined from the source code tags.
     *
     * @param	argv	an array of arguments that a test may use
    ...
    protected Status getPropsReady (String[] argv, Properties p) {
        ...
        //TestUtil.logTrace("**Got current testclass : " + this.getClass().getName());
        //TestUtil.logTrace("*** Got testclass property : " + p.getProperty("test_classname"));
        ...
        try {
            p.setProperty("harness.host", InetAddress.getLocalHost().getHostAddress());
        } catch (UnknownHostException e) {
            TestUtil.logErr("Could not get our hostname to send to the " + "server for testcase: "
                    + sTestCase);
    ...
    //}
    /**
     * This run method is the one that actually invokes reflection to
     * figure out and invoke the testcase methods.
     *
     * @param	argv	an array of arguments that a test may use
     * @param	p		properties that are used by the testcase
     * @return			a Javatest Status object (passed or failed)
    ...
    public Status run (String[] argv, Properties p) {
        ...
        if (sTestCase == null || sTestCase.equals("")) {
            ...
            // 			if ( !((this.getClass().getName()).equals((p.getProperty("test_classname").trim())))) {
            //				try {
            ...
            setupMethod = getSetupMethod(testClass);
            if (setupMethod == null)
            ...
            cleanupMethod = getCleanupMethod(testClass);
            ...
            if (setupMethod == null || runMethod == null || cleanupMethod == null)
                return  Status.failed("One of the test methods could not be" + "found for testcase: "
                        + sTestCase);
            try {
                ...
                setTestStatus(Status.failed("Could not execute setup method" + "for test case: "
                        + sTestCase), e);
            } catch (InvocationTargetException e) {
                setTestStatus(Status.failed("Test case throws exception: " + e.getTargetException().toString()),
                        e.getTargetException());
            } catch (RuntimeException e) {
                setTestStatus(Status.failed("Could not access the test case: " +
                        e.toString()), e);
            } catch (ThreadDeath t) {
                throw  t;
            } catch (Throwable t) {
            ...
            if (sTestStatus.getType() == Status.PASSED) {
                ...
                try {
                    ...
                    setTestStatus(Status.failed("Could not execute run method" +
                            "for test case: " + sTestCase), e);
                } catch (InvocationTargetException e) {
            ...
            try {
                ...
                setTestStatus(Status.failed("Could not execute cleanup method for test case: "
                        + sTestCase), e);
            } catch (InvocationTargetException e) {
    ...
    private String[] getAllTestCases (Properties p) throws SetupException {
        ...
        try {
            ...
            String sClientJavaName = sClientClassName.substring(sClientClassName.lastIndexOf('.')
                    + 1) + ".java";
            ...
            sRelativeTestDir = sRelativeTestDir.substring(sRelativeTestDir.indexOf("/")
                    + 1);
            //make sure we have a trailing "/"
            if (!sRelativeTestDir.endsWith("/"))
                sRelativeTestDir += "/";            /* Get public methods for this class
             * Loop through them to get methods that return void,
             * have no parameters, and contain "Test" in their name.
             */
            ...
            for (int ii = 0; ii < methods.length; ii++) {
                ...
                //Void.class.isAssignableFrom(returnType))
                {
                    ...
                    if ((sName.indexOf("Test") != -1 || sName.indexOf("test") !=
                            -1) && (sName.indexOf("Setup") == -1 && sName.indexOf("setup")
                            == -1) && (sName.indexOf("Cleanup") == -1 && sName.indexOf("cleanup")
                            == -1)) {
    ...
    /**
     * prints a string to the TestUtil log stream.  All tests should
     * use this method for standard logging messages
     *
    ...
    /**
     * prints a debug string to the TestUtil log stream.  All tests
     * should use this method for verbose logging messages.  Whether
     * or not the string is printed is determined by the last
     * call to the TestUtil setTrace method.
     *
     * @param	msg	string to print to the log stream
     */
    ...
    /**
     * prints a string to the TestUtil error stream.  All tests should
     * use this method for error messages
     *
    ...
    /**
     * This exception must be thrown by all implentations of EETest to
     * signify a test failure.
     * Overrides 3 printStackTrace methods to preserver the original stack trace.
     * Using setStackTraceElement() would be more elegant but it is not available
     * prior to j2se 1.4.
     *
     * @author	Kyle Grucci
     */
