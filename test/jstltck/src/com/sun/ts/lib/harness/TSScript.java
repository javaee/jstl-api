...
/*
 * $Id: TSScript.java 55453 2008-07-23 18:02:11Z ryano $
 */
...
public class TSScript extends com.sun.javatest.Script {
    ...
    public Status run(String[] args, TestDescription td, TestEnvironment env) {
        ...
        if (testDir.indexOf("interop") == -1 && sInteropDirection.equals("reverse")) {
            bIsRebuildableReverseTest = true;
            File fClassesVIBuiltDir = new File(sClassesVIBuiltDir);
            String[] sVIBuiltClasses = fClassesVIBuiltDir.list(ClassFilter.getInstance());

            if (sVIBuiltClasses == null || sVIBuiltClasses.length == 0) {
                String error = "TEST NOT RUN:  This test cannot be run until you" + TestUtil.NEW_LINE +
                        "rebuild the test classes and/or archives associated with it." + TestUtil.NEW_LINE +
                        "Please see the TCK documentation for more" + TestUtil.NEW_LINE +
        ...
        try {
            ...
            if (executionMode == ExecutionMode.DEPLOY) {
                if (ss.getDeploymentStatus().equals("failed")) {
                    ...
                    return Status.failed("An error occurred during the Deployment phase for tests in this directory.  This phase may include JMS topic/queue creation, JMS Factory Creation, and the actual deployment of any J2EE applications, components, and connector resources");
                } else {
            ...
            //We must reset all settings to forward because of the call above to recreate
            //the property manager with the new environment.  The call above is needed for every
            //test since a user may run the GUI and change jte file settings prior to running
            //another set of tests.  This also fixes bug# 4953862.   
            ss.swapSettings("forward");

            if (TestUtil.harnessDebug) {
                TestUtil.logHarnessDebug("TSScript - sInteropDirection:  " +
                        sInteropDirection);
            }
            if (sInteropDirection.equals("reverse")) {
                ss.swapSettings("reverse");
                if (TestUtil.harnessDebug) {
                    TestUtil.logHarnessDebug("Reversed settings for test:  " +
                            testName);
                }
            }
            ...
            if (executeArgs != null) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    if (sTemp.equals("-ap")) {
                        ...
                        if (next.equals("tssql.stmt") && sInteropDirection.equals("reverse")) {
            ...
            if (TestUtil.harnessDebug) {
                TestUtil.logHarnessDebug("** for JCK::testClassName= " +
                        executeClass);
            }
            ...
            if (sIsServiceTest.equals("yes")) {
                ...
                if (sVehicle.indexOf("servlet") != -1 ||
                        sVehicle.indexOf("web") != -1 ||
                        sVehicle.indexOf("jsp") != -1 ||
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        return Status.failed("Please supply values for webServerHost & webServerPort.  They are required to run service tests in a jsp or servlet.");
                    }
                }
            ...
            try {
                ...
                return Status.failed(
                        "TSScript:  Failed to create tstest.jte file");
            }
            ...
            //call SuiteSynchronizer.runAppclient which calls TSDeployment.runAppclient(executeargs
            ...
            if (sClientEar == null ||
                    sVehicle.equals("standalone") ||
                    sVehicle.indexOf("servlet") != -1 ||
                    sVehicle.indexOf("web") != -1 ||
                    sVehicle.indexOf("jsp") != -1 ||
                    sVehicle.indexOf("jsf") != -1) {
                ...
                if (sClientEar.indexOf("_vehicles") != -1) {
                    pExecProps.put("client_name",
                            ...
                            sClientEar.indexOf("_vehicles")) + "_" +
                            sVehicle + "_vehicle_client");
                } else {
                    pExecProps.put("client_name",
                            sClientEar.substring(sClientEar.lastIndexOf(File.separator) + 1,
            ...
            status = Status.failed(
                    "Can't load test: required interface not found");
        } catch (Exception e) {
    ...
    private String getVehicleArchiveName() {
        String[] sAppJarsArray = ProfileHelper.getArchives(TestUtil.srcToDist(testDir), sInteropDirection);
    ...
    private String getClientEarFile() {
        ...
        try {
            for (int ii = 0; ii < sAppJarsArray.length && sAppClientEar == null;
                    ii++) {

                if ((bIsRebuildableReverseTest && sAppJarsArray[ii].indexOf("vi_built_") == -1) ||
                        (!bIsRebuildableReverseTest && sAppJarsArray[ii].indexOf("vi_built_") != -1)) {
                    continue;
                }

                if (TestUtil.harnessDebug) {
                    TestUtil.logHarnessDebug("earFile = " + testDir +
                ...
                if (sIsServiceTest.equals("yes")) {
                    if (sAppJarsArray[ii].indexOf("_vehicles") != -1 ||
                        ...
                        sAppClientEar = testDir + File.separator +
                                sAppJarsArray[ii];
                        break;
                    }
                } else {
                    if (sAppJarsArray[ii].endsWith(".ear")) {
                        earFile = new JarFile(testDir + File.separator +
                                sAppJarsArray[ii]);
    ...
    //Set up the test-specific props file here and add it to the args
    //that are passed to EETest
    private String createTestPropertyFile(java.util.Properties p) throws
            Exception {
        ...
        if (!ctsTempDir.exists()) {
            if (!ctsTempDir.mkdirs()) {
                throw new Exception(
                        "Failed to create the testsuite's temporary working directory:  " + sTestDir);
            }
        }
    ...
    private String formatPath(String classpath) {
        ...
        try {
            String added = DeliverableFactory.getDeliverableInstance().getAdditionalClasspath(testDir);
            ...
            TestUtil.logHarness("Failed to get additional classpath for the following dist directory:  " + testDir);
        }
    ...
    protected Status invokeCommand(String key) {
        ...
        try {
            ...
            String sClassPathFromExecProps = pExecProps.getProperty(
                    "clientClasspath",
                    sClientClassesPath);
            TestUtil.logHarness("sClassPathFromExecProps = " +
            ...
            for (int ii = 0; ii < command.length; ii++) {
                if (isWindows() && command[ii].toUpperCase().startsWith("PATH=")) {
                    ...
                    String existingPath = command[ii].substring(
                            iEqualsIndex + 1);
                ...
                if (command[ii].startsWith("CLASSPATH=")) {
                    int iEqualsIndex = command[ii].indexOf("=");
                    String existingClassPath = command[ii].substring(
                            iEqualsIndex + 1);
                    command[ii] = "CLASSPATH=" + sClassPathFromExecProps +
                            formatPath(existingClassPath);

                    if (TestUtil.harnessDebug) {
                        TestUtil.logHarnessDebug("new classpath = " +
                                command[ii]);
                    }
                }
                if (command[ii].equals("-classpath")) {
                    command[ii + 1] = formatPath(command[ii + 1]) +
            ...
            String basic = "command: " + className + " " +
                    StringArray.join(args);
            ...
            t.printStackTrace();
            return Status.error("An unexpected error occured in TSScript's invokeCommand method" +
                    t.getMessage());
        }
    }
    ...
    private Status invokeClass(String className, String[] args,
        ...
        try {
            ...
            return Status.error("Bad class name `" + className + "', used in `" + env.getName() + "'");
            ...
            e.printStackTrace(logOut);
            return Status.error("Unexpected error trying to load command `" +
                    className + "': " + e);
        } catch (Throwable e) {
            e.printStackTrace(logOut);
        ...
        try {
            ...
            // negative tests
            return Status.failed(
                    "Unexpected exception while executing command " + className + ": " + e);
        } catch (Error e) {
            e.printStackTrace(logOut);
            // error reduced to failed in following line for benefit of
            // negative tests
            return Status.failed("Unexpected error while executing command " +
                    className + ": " + e);
        } catch (Throwable e) {
            e.printStackTrace(logOut);
