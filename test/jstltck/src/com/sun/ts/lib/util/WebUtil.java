...
/*
 * $Id: WebUtil.java 54204 2008-01-24 18:53:51Z ryano $
 */
...
public class WebUtil {
	...
	/**
	 * Parses a header line for an old-style cookie (not Set-Cookie2), 
	 * and stores the cookie in the cookies table.  Only the key and 
	 * value are stored.  Expected syntax:  "Set-Cookie: NAME=VALUE[;...]"
	 * The key is stored in upper-case.
	 *
	...
	/**
	 * Returns true if the status token for this response represents an
	 * error, or false if the status token indicates the page was retrieved
	 * okay.  Note that a redirection is not an error.
	 */
        public boolean isError() {
	    // According to RFC2616, all status tokens staring with 4xx or 5xx
	    // are errors.
	    return statusToken.startsWith( "4" ) || 
		statusToken.startsWith( "5" );
	}
    }

    /**
     * Converts a standard URL to a request.  For example, the string
     * "http://goodtimes:8000/testing" would be converted to 
     * "/testing".
     *
     * @param urlString The URL to convert
     * @return The resulting GET request
     * @exception MalformedURLException Thrown if the urlString does not 
     *     contain a valid URL.
     */
    ...
    /**
     * Sends a request to the web server.  A WebUtil.Response object is 
     ...
     * @param req The file to request
     *     (e.g. /jsp_dep_secContextRoot/jspSec.jsp)
     * @param postData If this is a POST request, the data to be posted,
     *     encoded in a Properties class.  null if no post data to be sent.
     * @param cookieList A list of cookies to send when requesting the page.
     *     null if no cookie list is to be sent.
	...
	Hashtable cookieList ) throws IOException
    {
    ...
    /**
     * Sends an authenticated request to the web server.  A 
     * WebUtil.Response object is returned with the response information.  
     *
     * @param method Can be either "GET" or "POST"
     * @param addr Address of web server
     * @param port Port of web server
     ...
     * @param cookieList A list of cookies to send when requesting the page.
     *     null if no cookie list is to be sent.
     * @param username The username for authentication, null if no 
     *     authentication required.
     * @param password The password for authentication, null if no
     *     authentication required.
     * @return WebUtil.Response object containing response information
     * @exception IOException Thrown if request could not be made
     */
	...
	try {
	    ...
	    requestURL = new URL( "http", hostname, port, 
		req ); 
    ...
    /**
     * Outputs a single line of text to the given output stream.  Appends
     * a \r\n automatically.
     * By adding a System.out.println here, you can easily echo what is being
