...
/*
 * $Id: TestUtil.java 53427 2007-05-17 17:36:44Z kgrucci $
 */
...
/**
 * TestUtil is a final utility class responsible for implementing logging
 * across multiple VMs.  It also contains many convenience methods for
 * logging property object contents, stacktraces, and header lines.
 *
 * @author	Kyle Grucci
 *
 */
public final class TestUtil {
    ...
    /**
     * used by TSTestFinder and TSScript to log output to
        ...
        synchronized (socketMutex) {
            ...
            try {
                ...
                synchronized (vBuffereredOutput) {
                    ...
                    //System.out.println("ADDED THIS STRING TO BUFFERED OUT:  " + info.sOutput);
                }
            }
            flushStream();
        }
    ...
    /**
     * This static method must be called once by each new remote VM.
     * Once called, a socket connection is created back to the host
     * running the test harness.  All calls to logMsg, logErr, and
     * logTrace are immediately sent back to the harness host.
     *
     * @param	p	properties containing harness host, port, and trace flag
     * @exception	RemoteLoggingInitException	if an exception occurs
     *		        while the server side is setting up the socket
        ...
        synchronized (socketMutex) {
            try {
                ...
                if (true) {
                    ...
                    traceflag = Boolean.valueOf(p.getProperty("harness.log.traceflag",
                            "true")).booleanValue();
                    hostOfHarness = p.getProperty("harness.host");
                    portOfHarness = Integer.parseInt(p.getProperty("harness.log.port",
                            "2000"));
                ...
                try {
                    //System.out.println("INIT_CALLED AND TRYING TO ESABLISH CONN. AFTER IOEXCEPTION");
                    traceflag = Boolean.valueOf(p.getProperty("harness.log.traceflag",
                            "true")).booleanValue();
                    hostOfHarness = p.getProperty("harness.host");
                    portOfHarness = Integer.parseInt(p.getProperty("harness.log.port",
     ...
     * between the harness and the tests and calls TestUtil logMsg and logTrace
     * to do the right thing inside of the JavaTest VM.  These calls will call
     * to our logHarness methods.
     */
    ...
    /**
     * This static method must be called once by the harness VM.
     * Once called, a serversocket begins listening for Remote VMs to
     * connect on the port specified by harness.log.port.
     *
     * @param	p	properties containing harness trace flag
     */
	...
	logTrace("#######  Value of harness.socket.retry.count is \"" + retry + "\"");	
	...
	logTrace("#######  Actual bind value of harness.log.port is \"" + port + "\"");	
	return result;
    }
    ...
    /**
     * This static method must be called once by the harness VM in order
     * to set the output and error streams and the name of the current test.
     *
     * @param	sTestName	the currently running testname as specified in the
     *				source code tags
     * @param	outStream	stream printed to by the logMsg and logTrace
     *				methods
    ...
    /**
     * prints a string to the log stream.  All tests should use this method
    ...
    public static void logMsg (String s) {
        if(iWhereAreWe == VM_JAVATEST)
        {
            ...
            writeObject(tri);            /*try
             ...
             objectOutputStream.writeObject(tri);
             objectOutputStream.flush();
             //System.out.println("successfully wrote to objectOutputStream");
             }
             }
             catch(Exception ex)
             {
             //System.out.println("got exception trying to write to objectOutputStream");
    ...
    /**
     * turns on/off debugging.  Once on, all calls to the logTrace method
     * result in messages being printed to the log stream.  If off, all
     * logTrace calls are not printed.
     *
     * @param	b	If <code>true</code>, debugging is on.  If false,
    ...
    /**
     * prints a debug string to the log stream.  All tests should use this
     * method for verbose logging messages.  Whether or not the string is
     * printed is determined by the last call to the setTrace method.
     *
     * @param	s	string to print to the log stream
     */
    ...
    /**
     * Prints a debug string as well as the provided Throwable's
     * stacktrace.  Use this if certain exceptions are only
     * desired while tracing.
     *
     * @param s - string to print to the log stream
        ...
        if (traceflag) {
            ...
            else
            {
                if (iWhereAreWe == VM_HARNESS) {
                    ...
                    TestReportInfo tri = new TestReportInfo("SVR-TRACE: " + s, OUTPUT_STREAM,
                        DEBUG_OUTPUT_LEVEL, null);
                    writeObject(tri);
                }
                if(t != null)
                {
    ...
    /**
     * prints an error string to the error stream.  All tests should
    ...
    public static void list (Properties p) {
        ...
        while (e.hasMoreElements()) {
            ...
            sb.append(key).append("=").append(p.getProperty(key)).append(TestUtil.NEW_LINE);
        }
    ...
    /**
     * Convience method to handle sucking in the data from
     * a connection.
     */
    ...
    /**
     * One shot method to get Properties directly from a
     * URLConnection.
     */
    ...
    /**
     *  Parse a the table name from the ddl string such as:
     *  "create table foo" or "delete from foo"
    ...
    public static String replaceLastSrc (String src, String replacement) {
        ...
        if (iStart != -1) {
            ...
            if(harnessDebug) {
                TestUtil.logHarnessDebug("Converted " + replacement + " dir = " + sbToConvert);
            }
        }
