 ...
 */
/*
 * $Id: AssertionMapper.java 52683 2007-02-12 02:25:42Z lschwenk $
 */
...
public final class AssertionMapper {
    ...
    //xmlFileNames that parser has failed to load, and thus will not load it again.
	...
	if(assertionDir == null)
	     assertionDir = TSTestFinder.TS_HOME + File.separator +
	       "internal" + File.separator + "docs";
	...
	String[] s = {
	 "JMS:SPEC:130.3", "EJB:JAVADOC:27", "JMS:SPEC:130.19",
	...
	if(assertionIds == null || assertionIds.length() == 0) {
	    System.out.println("WARN: no value for assertion_ids ("
		+ filePath + ")");
	    return NO_DESC;
	}
	...
	Set addedAssertions = new HashSet();  //avoid one assertion to be added multiple times
	StringBuffer retval = new StringBuffer();
	for (int i = 0; i < idArray.length; i++) {
	    String aDescription = getDescription0(idArray[i], filePath, addedAssertions);
     ...
     * @param filePath the full path to the test source code that contained the
     * assertion ids.  For example, /files/ts/src/com/sun/ts/tests/jsp/syntax/Client.java.
     ...
     * assertion, null or empty string is returned.  A complete description includes
     * description of all of its ancestors, its own description, and that of all
     * of its descendents.
     */
...
//	extends DefaultHandler {
	...
	public void load(String xmlFileName){
		...
		System.err.println("%%%%%%%%%% parentDir set to (resolver uses this) \"" + parentDir + "\" %%%%%%%%%");
				   
		InputSource is = new InputSource(new FileInputStream(
		    new File(assertionDir, fn)));
	...
	/*
	 * Implementation of the entity resolver interface.  This allows CTS developers
	 * to use entity declarations in their assertion docs provided the assertion doc
	 * (the referencing doc) and the referenced docs are in the same directory.
	 */
	public InputSource resolveEntity (String publicId, String systemId) {
	    ...
	    String possibleLocation = assertionDir + File.separator + parentDir + File.separator + fileName;
	    ...
	    System.err.println("%%%%%%%% Entity Resolver returning \"" + result + "\"");
...
//                  throws SAXException {
	if(localName.equalsIgnoreCase("assertions")) {
	    ...
	    b_assertion = true;
        } else if(localName.equalsIgnoreCase("id") && b_assertions && b_assertion) {
	    b_id = true;
        } else if(localName.equalsIgnoreCase("description") && b_assertions && b_assertion) {
	    b_description = true;
        } else if(localName.equalsIgnoreCase("sub-assertions") && b_assertions && b_assertion) {
	    putIdAndDescription();
...
//                throws SAXException {
	...
	if(b_id) {
	    ...
	    } else {  // some descriptions have formatting elements <code>, <tt>,which
	...
	if(currentDescription == null || currentDescription.length() == 0) {
	    System.out.println("WARN: null description in " + xmlFileName);
