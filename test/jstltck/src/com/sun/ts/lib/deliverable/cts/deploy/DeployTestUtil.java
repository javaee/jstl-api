...
/*
 * $Id: DeployTestUtil.java 52858 2007-03-09 00:48:22Z nrodin $
 */
...
public class DeployTestUtil {
  ...
  public static final String DEPLOYMENT_MANIFEST_ENTRY_NAME = 
  ...
  /**
   *
   * Construct a DeployTestUtil object.  Load the DeploymentFactories from
   * the provided jarfile, and get a DeploymentManager instance.
   *
   */
  ...
  public DeploymentManager getDeploymentManager()
    ...
    if(dm == null) {
      ...
      dm = dfmanager.getDeploymentManager(dmp.getURI(), dmp.getUname(), dmp.getPasswd());
      TestUtil.logHarnessDebug("$$$$$$$$$$$$$$$[" + dmp.getUname() + ", " + dmp.getPasswd() + "]");
  ...
  public Vector getDeploymentFactoryClassNames() throws Exception {
    ...
    try {
      ...
      throw new Exception("Could not contruct file object: " + dmp.getJarFile());
    }
    ...
    Manifest mf = new java.util.jar.JarFile(deployPlatformJarFile).getManifest();
  ...
  private void loadDMFactories() throws Exception {
    ...
    TestUtil.logHarness("DeployTestUtil.loadDMFactories using jarfile: " + dmp.getJarFile());
  ...
  private void loadDeploymentFactories() throws Exception {
    ...
    // This should be rewritten to get multiple class names from the manifest
    TestUtil.logHarness("Looking for manifest entry: " + 
		      DEPLOYMENT_MANIFEST_ENTRY_NAME);
    ...
    try{
      ...
      TestUtil.logHarness("loadDeploymentFactories about to load classes :" + dfClassNames);
      Thread.currentThread().setContextClassLoader(urlClassLoader);

      // Load all classes
      for(int classNameIndex=0; classNameIndex < classNames.size();
	  classNameIndex++) {
	...
	if (df instanceof DeploymentFactory) {
	  ...
	  TestUtil.logMsg("Registered DeploymentFactory: " + df.getDisplayName());
	} else {
  ...
  public boolean checkIfModuleExists(TargetModuleID moduleToCheck, TargetModuleID[] modulesList){
      ...
      if (moduleToCheck == null) { return exists; }
      int numMods = (modulesList == null) ? 0 : modulesList.length;
      for (int modIndex = 0; modIndex < numMods; modIndex++) {
	  if (moduleToCheck.getModuleID().equals(modulesList[modIndex].getModuleID())){
	      // Also check for same targets.
	      if(moduleToCheck.getTarget().getName().equals(modulesList[modIndex].getTarget().getName())){
		  exists = true;
		  break;
	      }
	  }
  ...
  /*
   * If the operation is successful then TargetModuleID is a valid module id.  Else it will be null
   **/
  public TargetModuleID distributeModuleArchive(Target[] targets, File moduleArchive, File deploymentPlan) throws Exception{
    ...
    if(resultModuleIDs.length < 1){
      // There should be atleast one target module id if progress is successfully complete
      return null;
    }
  ...
  public boolean testDistributeModule(ModuleType moduleType, File moduleArchive, File deploymentPlan, String archiveType, boolean oneDotFiveApi) throws Exception{
    ...
    if(archiveType.equals(FILE_ARCHIVE)){
      distributedModuleID = distributeModuleArchive(new Target[]{targetToDeploy},moduleArchive,deploymentPlan);
    }else{
      ...
      distributedModuleID = distributeModuleStream(new Target[]{targetToDeploy}, 
          moduleType, moduleStream, planStream, oneDotFiveApi);
    }
    ...
    boolean moduleExistsBeforeDistribute = checkIfModuleExists(distributedModuleID,moduleIDsBeforeDistribute);
    boolean moduleExistsAfterDistribute = checkIfModuleExists(distributedModuleID,moduleIDsAfterDistribute);
  ...
  public boolean testStartModule(ModuleType moduleType, File moduleArchive, File deploymentPlan, String archiveType) throws Exception{
    ...
    if(archiveType.equals(FILE_ARCHIVE)){
      ...
      distributedModuleID = distributeModuleStream(new Target[]{targetToDeploy},
          moduleType, new FileInputStream(moduleArchive),new FileInputStream(deploymentPlan),
          true);
    }
    // If the distributedModuleID == null, the test is failed
    if(distributedModuleID == null)
  ...
  public boolean testStopModule(ModuleType moduleType, File moduleArchive, File deploymentPlan, String archiveType) throws Exception{
    ...
    if(archiveType.equals(FILE_ARCHIVE)){
      ...
      distributedModuleID = distributeModuleStream(new Target[]{targetToDeploy},
          moduleType, new FileInputStream(moduleArchive),new
          FileInputStream(deploymentPlan), true);
    }
    // If the distributedModuleID == null, the test is failed
    if(distributedModuleID == null)
      return false;
    ...
    if(!startedModuleID.getModuleID().equals(stoppedModuleID.getModuleID()))
      return false;
    boolean modExistsInRunningMods = checkIfModuleExists(stoppedModuleID,dm.getRunningModules(moduleType, new Target[]{targetToDeploy}));
    boolean modExistsInNonRunningMods = checkIfModuleExists(stoppedModuleID,dm.getNonRunningModules(moduleType, new Target[]{targetToDeploy}));
  ...
  public boolean testRedeployModule(ModuleType moduleType, File moduleArchive, File deploymentPlan, String archiveType) throws Exception{
    ...
    if(archiveType.equals(FILE_ARCHIVE)){
      ...
      distributedModuleID = distributeModuleStream(new Target[]{targetToDeploy}, 
          moduleType, new FileInputStream(moduleArchive),
    ...
    if(archiveType.equals(FILE_ARCHIVE)){
      redeployedModuleID = redeployModuleArchive(new TargetModuleID[]{startedModuleID},moduleArchive,deploymentPlan);
    }else{
      redeployedModuleID = redeployModuleStream(new TargetModuleID[]{startedModuleID},new FileInputStream(moduleArchive),new FileInputStream(deploymentPlan));
    }
  ...
  public boolean testUndeployModule(ModuleType moduleType, File moduleArchive, File deploymentPlan, String archiveType) throws Exception{
    ...
    if(!undeployedModuleID.getModuleID().equals(distributedModuleID.getModuleID()))
      return false;
    if(checkIfModuleExists(undeployedModuleID,dm.getAvailableModules(moduleType, new Target[]{targetToDeploy})))
      return false;
    else return true;       
  }
  ...
  public boolean testTargetModuleID(File moduleArchive, File deploymentPlan, String archiveType){
     ...
     * executed command is actually finished?  First, Prakash mentioned that
     * there is a case where it is possible for an executed command to finish
     * before the user is even returned the progress object.  In this case if
     * we register a ProgressListener with the ProgressObject and handle
     * events within the ProgressListener's handleProgressEvent() we may miss the
     * event that told us the executed command had finished.  Second, again
     * according to Prakash, the spec does not specify how many events are
     * generated when executing a command.  For instance, if we call stop and
     * pass 4 TargetModuleID objects, there is no place in the spec that tells
     * us how many events we should expect.  There could be a set of one or
     * more events for each target or a single event for all the targets.  Since
     * the spec doesn't define this, we shouldn't assume.
     *
     * Having said that we decided to simply poll the progress object and ask
     * it when it thinks it is finished.  This will be denoted by a COMPLETED
     * or FAILED state.  If the state is FAILED we can use the 
     ...
     * for against the targets returned by getResultTargetModuleIds.  The doc
     * for getResultTargetModuleIds says it only returns the IDs of the targets
     * that successfully completed the specified command.  See the OperationStatus
     * class for details on determining the failed targets.
     */
	...
	TestUtil.logTrace("Maximum wait time for deployment is " + result + " minutes.");
	return result;
    }
	...
	(Target[] targets, InputStream moduleArchive, InputStream deploymentPlan) throws Exception
    {
	ProgressObject progress = null;
	try {
	    progress = getDeploymentManager().distribute(targets, moduleArchive, deploymentPlan);
	} catch (Exception e) {
	    e.printStackTrace();
	...
	return result;
     ...
     public OperationStatus distributeModuleFiles
        (Target[] targets, File moduleArchive, File deploymentPlan) throws Exception
    {
        ProgressObject progress = null;
        try {
            progress = getDeploymentManager().distribute(targets, moduleArchive, deploymentPlan);
	...
	OperationStatus result = commandStatus(convertTargets(modulesIDsToUndeploy), progress);
	delay();
	return result;
    }  
