...
/*
 * $Id: StandardDeployment14.java 55788 2008-09-08 12:28:15Z phendley $
 */
...
/**
 *
 * This class implements the TSDeploymentInterface.  It does so by
 * using the 88 API's.  As of J2EE 1.4, this class should be the standard
 * implementation of TSDeploymentInterface.  There will be no need of
 * alternative implentations.  All implementation specific code now exists in
...
public class StandardDeployment14 implements TSDeploymentInterface {
	...
	TestUtil.logHarness("StandardDeployment14:  Using porting set #" + iPortingSet);
	...
	try	{
		...
		deployStateFile = TEMP_DIR + File.separator + DEPLOYED_MODULES_FILE + "_" + portingString + ".ser";
	    ...
	    Runtime.getRuntime().addShutdownHook(new Thread() {
			...
			TestUtil.logHarness("IN SHUTDOWNHOOK state file is : \"" + deployStateFile + "\"");
			dumpDeployedMods(htDeployedModules);
			...
			if (deployedModules.size() != 0) {
			    ...
			    for (Iterator i = deployedModules.keySet().iterator();
				 i.hasNext();) {
				...
				try {
				    ...
				    TestUtil.logHarness("Unexpected exception while" +
							" undeploying application during shutdown. " +
							"Cause: " + tde.getMessage());
				}
	    ...
	    e.printStackTrace();
	    TestUtil.logHarness("Creation of TSDeployment2 implementation instance failed." +
	...
	try {
	    ...
	    OperationStatus status;
            ...
            if(plan instanceof InputStream) {
 	        ...
 	        status = dtu.distributeModuleStreams(targets, new FileInputStream(earfile), (InputStream)plan);
	    ...
	    TestUtil.logHarness("$$$$$$$$$$ Deployment SUCCEEDED for \"" + earfile + "\"");

	    // Allow the licensee an opportunity to examine the returned ProgressObject and
	    ...
	    htDeployedModules.put(getAppName(sArchive), status.getDeployedTargetIDs());
        ...
        if(sArchive.indexOf("xa" + File.separator + "ee" +
				File.separator + "tsr" + 
		...
		return dep2.getClientClassPath(status.getDeployedTargetIDs(), info, dtu.getDeploymentManager());
	    }
	} catch (Exception e) {
	    e.printStackTrace();
	    throw (TSDeploymentException) 
                new TSDeploymentException("Deployment Failed.").initCause(e);
	}
    }

    public void undeploy(String moduleId) throws TSDeploymentException {
	ModuleType moduleType = getModuleType(moduleId);
	TargetModuleID[] idsToUndeploy = (TargetModuleID[])htDeployedModules.get(getAppName(moduleId));
    
	TestUtil.logHarnessDebug("$$$$$$$$$$$$$ idsToUndeploy.length = " + idsToUndeploy.length);
	TestUtil.logHarnessDebug("$$$$$$$$$$$$$ Undeploying Module ID \"" + idsToUndeploy[0].getModuleID() + "\"");

	if (idsToUndeploy == null) {
	    TestUtil.logHarness("idToUndeploy is null.  Assuming that the module is not currently deployed");
	    return;
	} else {
	...
	try {
		...
		TestUtil.logHarness("Stop failed for one or more targets while undeploying module \"" + moduleId + "\"");
	    }
	    ...
	    if (status.isFailed()) {
		TestUtil.logHarness("Undeploy failed for one or more targets \"" + status.errMessage() + "\"");
	    }
	...
	if (sArchive.indexOf("connector" + File.separator + "deployment" + 
			    File.separator + "ejb_Deployment.ear") != -1 ||
	  sArchive.indexOf("xa" + File.separator + "ee" +
			    File.separator + "tsr" + 
			    File.separator + "ejb_Tsr.ear") != -1) {
	    ...
	    TargetModuleID[] modulesToUndeploy =
		(TargetModuleID[])(htDeployedModules.get(getAppName(sArchive)));
	    if (modulesToUndeploy.length == 0) {
		TestUtil.logHarness("undeploy failed for application \"" + sArchive + "\"");		
	...
	try {
	    ...
	    TargetModuleID[] targets = (TargetModuleID[])(htDeployedModules.get(getAppName(sRarFileName)));
	...
	//If rar files now have runtime info, we will have to change the harness code
	// to call deploy if we are using the StandardDeployment class.  Our deploy
	// method in here can check for rars and handle the connection factory logic.
	...
	if(isConnectorDeployed(p)) {
	    ...
	    // targets must have a non-empty list of TargetModuleIDs since isConnectorDeployed just checked it.
	    TargetModuleID[] targets = (TargetModuleID[])(htDeployedModules.get(getAppName(rarFile)));
	    dep2.removeConnectionFactory(targets, p);
	    p.setProperty("ear_file", rarFile);
	    undeploy(p);
	} else {
	    TestUtil.logHarness("StandardDeployment14.undeployConnector() - " + "not undeploying.");
	}
    }
	...
	try  {
	    ...
	    TestUtil.logHarnessDebug("StandardDeployment14.initDeployTestUtils() + sPropNum = " + sPropNum);
	...
	try {
	    ...
	    e.printStackTrace();
	    TestUtil.logHarness("Exception loading DeploymentFactoryManager factories: " + e);
    ...
    /*
	 *  This method is used to update our local hashtable (htDeployedModules)
	 *	of deployed modules with the latest information from the targets
	 *	that we've deployed them to.
	 */
	...
	for (int i = 0; i < targets.length; i++) {
	    TestUtil.logHarnessDebug("[" + targets[i].getDescription() + ", " + targets[i].getName() + "]");
	}
	...
	while (iter.hasNext()) {
	    ...
	    TestUtil.logHarnessDebug("****** targModIDs.length = " + targModIDs.length);
	    TestUtil.logHarnessDebug("TargetModuleID[]");
	    for (int i = 0; i < targModIDs.length; i++) {
		TestUtil.logHarnessDebug("[" + targModIDs[i].getModuleID() + ", " + targModIDs[i].getTarget().getDescription() +
				   ", " + targModIDs[i].getTarget().getName() + "]");
	    }

	    TargetModuleID[] savedIDs = (TargetModuleID[])htDeployedModules.get(modName);
		...
		if (isValidTargetID(targModIDs[i], savedIDs)) {
		    TestUtil.logHarnessDebug("&&&&&&&& Adding = " + targModIDs[i].getModuleID());
		    targModIDsList.add(targModIDs[i]);
		    TestUtil.logHarnessDebug("&&&&&&&& targModIDsList.size() is " + targModIDsList.size());
		    break;
		}
	    }
            if (targModIDsList.size() > 0) {
	        TargetModuleID[] validSavedIDs =
	    	    (TargetModuleID[])(targModIDsList.toArray(new TargetModuleID[targModIDsList.size()]));
                result.put(modName, validSavedIDs);
            }
            else
            {
                TestUtil.logHarnessDebug("$$$$$ We did not find any valid IDs :  " + modName);
            }
	}
	...
	try {
		...
		(new FileOutputStream(deployStateFile));
	    oout.writeObject(newMap);
	} catch (IOException e) {
	    throw e;
	} finally {
	    try { oout.close(); } catch (Exception ee) {}
	}
    }
	...
	if (inFile.isFile()) {
		...
		oin = new ObjectInputStream
		    (new FileInputStream(deployStateFile));
	...
	public boolean equals(Object obj) {
		...
		result = (description == null) ? that.getDescription() == null : description.equals(that.getDescription());
		result = result && ((name == null) ? that.getName() == null : name.equals(that.getName()));
	    }
	    return result;
	}
    }
	...
	public boolean equals(Object obj) {
		...
		result = (moduleID == null) ? that.getModuleID() == null : moduleID.equals(that.getModuleID());
		result = result && ((target == null) ? that.getTarget() == null : target.equals(that.getTarget()));
	    }
	    return result;
	}
    }
